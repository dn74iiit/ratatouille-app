...
- **Live Government Mandi Data & Linear Programming**: Instead of static pricing, the system actively cross-references the required ingredients against live agricultural market (Mandi) data provided by the government. To find the optimal combination of ingredients that meets both the chemical requirements of the dish and the financial limits of the user, the backend utilizes `scipy.optimize.linprog`. This models the recipe generation as an optimization problem where the objective function is to minimize cost—using real-time Mandi commodity rates—while satisfying the constraints of texture, moisture, and binding capacity.
